...
(with-eval-after-load 'shr ; lazy load is very important, it can save you a lot of boot up time
  ...
  (setq shrface-toggle-bullets nil) ; Set Non-nil if you do not like headline bullets
...
(with-eval-after-load 'nov
  ...
  (define-key nov-mode-map (kbd "C-i") 'shrface-links-counsel)
  (define-key nov-mode-map (kbd "C-o") 'shrface-headline-counsel))
...
(with-eval-after-load 'eww
  ...
  (define-key eww-mode-map (kbd "C-i") 'shrface-links-counsel)
...
**** =shrface-links=, =shrface-links-counsel= and =shrface-links-helm=
***** =M-x= =shrface-links= to list all URLs and classify them. You can easily go to the
...
***** =M-x= =shrface-links-counsel= to list all URLs with =counsel=.
...
***** =M-x= =shrface-links-helm= to list all URLs with =helm=.
- =TAB= to preview the link
- =RET= to goto the link

**** =shrface-headline-counsel= and =shrface-headline-helm=

***** =M-x= =shrface-headline-counsel= to list all headlines with =counsel=.
...
***** =M-x= =shrface-headline-helm= to list all headlines with =helm=.
- =TAB= to preview the headline
- =RET= to goto the headlin

**** =shrface-next-headline= and =shrface-previous-headline=
...
**** Toggle headline bullets

The quick way to toggle(disable/enable) headline bullets:
#+BEGIN_SRC emacs-lisp
