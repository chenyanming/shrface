...
*** Enable the shrface
Require the package, and you can enable all faces expect the experimental face(s):
...
For example, to enable shrface-mode to =eww= or =nov=
#+BEGIN_SRC emacs-lisp
  (with-eval-after-load 'eww
    (add-hook 'eww-after-render-hook 'shrface-mode)) ; this will affect eww, dash-docs and mu4e
...
PS: Since =nov-shr-rendering-functions= is not a customizable variable. Therefore,
this setting is not included into the package.
...
Please notice, =org-show-children= are not 100% supported, since the =regexp-fmt= is
 ...
 directly). But you can use [[https://github.com/tj64/outline-magic][outline-magic]] to support better outline cycle
 feature.
